...
# import numpy to read in dnase peak calls
np <- import("numpy")

# load melanoma DNAseSeq calls
skcm_dnase <- np$load("/home/timmonsc/testing/data_SKCM_w1e6.npz")
skcm_dnase$files
hist_counts <- data.frame("hist_counts" = t(skcm_dnase$f[['hist_counts']]))
targets <- data.frame("targets" = t(skcm_dnase$f[['targets']]))
dnase_data <- cbind(hist_counts, targets)
dnase_data <- dnase_data[1:3113,]
dnase_data['mb_bin'] <- rep(1:3113)

# Automates testing
# Input folder containing signature profiles for a cancer type and generic 1 Mb binned genome
...
densityTest <- function(type, blank_genome) {
  pvals <- data.frame()
        ...
        changepoints_all <- summarizeChangepoints(get(temp), i) %>%
            ...
            if (nrow(changepoints_all) > 0) {
              ...
              changepoints_all <- rbind(changepoints_full, changepoints_truc) %>%
                dplyr::arrange(cpPos1)
        null_dist <- randomKS(get(temp), dnase_data, blank_genome)

          all <- sampleKS(changepoints_all, dnase_data) %>%
             dplyr::mutate(
                            "sampleID" = i,
                            "null_before" = 1,
                            "null_after" = 1)
...
# Find the DNAseSeq peak distributions within random changepoint-containing segment
...
# Return p-values
randomKS <- function(traj, dnase_data, blank_genome) {

  n = 10000

  if (nrow(dnase_data) > 0) {
    ...
    before_segment_vals <- apply(before_segment, 1, function(x){dnase_data$hist_counts[(which.min(abs(x[1]-dnase_data$mb_bin))):((which.min(abs((x[2]+1)-dnase_data$mb_bin)))-1)]})
    cp_segment_before_vals <- apply(cp_segment_before, 1, function(x){dnase_data$hist_counts[(which.min(abs(x[1]-dnase_data$mb_bin))):((which.min(abs((x[2]+1)-dnase_data$mb_bin)))-1)]})
    cp_segment_after_vals <- apply(cp_segment_after, 1, function(x){dnase_data$hist_counts[(which.min(abs(x[1]-dnase_data$mb_bin))):((which.min(abs((x[2]+1)-dnase_data$mb_bin)))-1)]})
    after_segment_vals <- apply(after_segment, 1, function(x){dnase_data$hist_counts[(which.min(abs(x[1]-dnase_data$mb_bin))):((which.min(abs((x[2]+1)-dnase_data$mb_bin)))-1)]})
...
# Return p values
sampleKS <- function(changepoints, dnase_data) {
  ...
  for (n in 1:nrow(changepoints)) {
    ...
    # find mean and sd gene hist_counts in the middle of segment before changepoint
    before_segment1 <- which.min(abs((midpoints_before[n]-(lengths_before[n]/2))-dnase_data$mb_bin))
    before_segment2 <- which.min(abs((midpoints_before[n]+(lengths_before[n]/2))-dnase_data$mb_bin))
    before_segment <- dnase_data$hist_counts[before_segment1:before_segment2]
    before_cp1 <- which.min(abs((midpoints_within[n]-(lengths_before[n]/2))-dnase_data$mb_bin))
    before_cp2 <- which.min(abs((midpoints_within[n]+(lengths_before[n]/2))-dnase_data$mb_bin))
    before_cp <- dnase_data$hist_counts[before_cp1:before_cp2]
    # find mean and sd gene hist_counts in the middle of segment after changepoint
    after_segment1 <- which.min(abs((midpoints_after[n]-(lengths_after[n]/2))-dnase_data$mb_bin))
    after_segment2 <- which.min(abs((midpoints_after[n]+(lengths_after[n]/2))-dnase_data$mb_bin))
    after_segment <- dnase_data$hist_counts[after_segment1:after_segment2]
    after_cp1 <- which.min(abs((midpoints_within[n]-(lengths_after[n]/2))-dnase_data$mb_bin))
    after_cp2 <- which.min(abs((midpoints_within[n]+(lengths_after[n]/2))-dnase_data$mb_bin))
    after_cp <- dnase_data$hist_counts[after_cp1:after_cp2]
...
main <- function() {
    ...
    density_test <- densityTest(type, blank_genome)
    write.csv(density_test, file=paste0("/home/timmonsc/testing/",type,"_dnasepvals.txt"), row.names=FALSE)
}
