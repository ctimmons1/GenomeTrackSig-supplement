...
# Automates testing
# Input folder containing signature profiles for a cancer type and corresponding TCGA code
...
atacTest <- function(type, tcga_code) {
  pvals <- data.frame()
  file_ids <- read.delim(paste0("/home/timmonsc/testing/bsub_logs/",type,"_ids.txt"), header=FALSE)
  colnames(file_ids) <- c("id")

  atac_tidy <- base::suppressWarnings(makeAtac(paste0("/home/timmonsc/testing/",tcga_code,"_peakCalls.txt")))
        ...
        if (!is.null(get(temp)[[2]])||!is.null(get(temp)[[6]])||!is.null(get(temp)[[10]])||!is.null(get(temp)[[14]])||!is.null(get(temp)[[18]])) {
          changepoints_all <- summarizeChangepoints(get(temp), i) %>%
            ...
            if (nrow(changepoints_all) > 0) {
              ...
              null_dist <- randomKS(get(temp), atac_tidy)

              kirc_ids <- readr::read_csv("/home/timmonsc/testing/kirc_ids.csv")
              kirp_ids <- readr::read_csv("/home/timmonsc/testing/kirp_ids.csv")
              if (tcga_code == "KIRC") {
                if (!fileID %in% kirc_ids$id) {
                  all <- data.frame()
                }
                else {
                  all <- sampleKS(changepoints_all, atac_tidy) %>%
...
# make dataset of ATAC-Seq peak calls given a file of
# peak calls (obtained from Corces and Granja et al. 2018 https://www.science.org/doi/10.1126/science.aav1898)
makeAtac <- function(path) {
  atac <- read.delim(path)
  atac_tidy <- atac %>%
    separate(seqnames, into = c("x1", "chr"), sep="r") %>%
    select(chr, start, end, score) %>%
    mutate(chr = dplyr::case_when(chr == "X" ~ 23,
                                  chr == "Y" ~ 24,
                                  TRUE ~ as.numeric(chr)),
...
# Find the ATAC peak distributions within random changepoint-containing segment
...
# Return p-values
randomKS <- function(traj, atac_tidy) {
  ...
  if (nrow(atac_tidy) > 0) {
    ...
    before_segment_vals <- apply(before_segment, 1, function(x){atac_tidy$score[(which.min(abs(x[1]-atac_tidy$mb_bin_start))):((which.min(abs((x[2]+1)-atac_tidy$mb_bin_end)))-1)]})
    cp_segment_before_vals <- apply(cp_segment_before, 1, function(x){atac_tidy$score[(which.min(abs(x[1]-atac_tidy$mb_bin_start))):((which.min(abs((x[2]+1)-atac_tidy$mb_bin_end)))-1)]})
    cp_segment_after_vals <- apply(cp_segment_after, 1, function(x){atac_tidy$score[(which.min(abs(x[1]-atac_tidy$mb_bin_start))):((which.min(abs((x[2]+1)-atac_tidy$mb_bin_end)))-1)]})
    after_segment_vals <- apply(after_segment, 1, function(x){atac_tidy$score[(which.min(abs(x[1]-atac_tidy$mb_bin_start))):((which.min(abs((x[2]+1)-atac_tidy$mb_bin_end)))-1)]})
...
# Find the ATAC peak distributions within each changepoint-containing segment
# and regions surrounding changepoints
# Perform KS test between changepoint region and surrounding regions
# Return p values
sampleKS <- function(changepoints, atac_tidy) {
  ...
  for (n in 1:nrow(changepoints)) {
    ...
    before_segment1 <- which.min(abs((midpoints_before[n]-(lengths_before[n]/2))-atac_tidy$mb_bin_start))
    before_segment2 <- which.min(abs((midpoints_before[n]+(lengths_before[n]/2))-atac_tidy$mb_bin_start))
    before_segment <- atac_tidy$score[before_segment1:before_segment2]
    before_cp1 <- which.min(abs((midpoints_within[n]-(lengths_before[n]/2))-atac_tidy$mb_bin_start))
    before_cp2 <- which.min(abs((midpoints_within[n]+(lengths_before[n]/2))-atac_tidy$mb_bin_start))
    before_cp <- atac_tidy$score[before_cp1:before_cp2]
    # find dist of atac scores after cp vs within cp
    after_segment1 <- which.min(abs((midpoints_after[n]-(lengths_after[n]/2))-atac_tidy$mb_bin_start))
    after_segment2 <- which.min(abs((midpoints_after[n]+(lengths_after[n]/2))-atac_tidy$mb_bin_start))
    after_segment <- atac_tidy$score[after_segment1:after_segment2]
    after_cp1 <- which.min(abs((midpoints_within[n]-(lengths_after[n]/2))-atac_tidy$mb_bin_start))
    after_cp2 <- which.min(abs((midpoints_within[n]+(lengths_after[n]/2))-atac_tidy$mb_bin_start))
    after_cp <- atac_tidy$score[after_cp1:after_cp2]
...
summarizeChangepoints <- function(trajectory, sampleID) {
  ...
  if (nrow(cpPos) > 0) {
    ...
    if (nrow(cpPos) == 1) {
      ...
      # mb_bin_before <- blank_genome %>%
        # dplyr::filter(start_chrom == cpPos$before_chrom[1] & start == cpPos$start_before[1])
      cpPos$mb_bin_start[1] <- mb_bin_start$mb_bin
      # cpPos$mb_bin_before[1] <- mb_bin_before$mb_bin
      mb_bin_end <- blank_genome %>%
        dplyr::filter(end_chrom == cpPos$end_chrom[1] & end == cpPos$end[1])
      # mb_bin_after <- blank_genome %>%
      #   dplyr::filter(end_chrom == cpPos$after_chrom[1] & end == cpPos$end_after[1])
      cpPos$mb_bin_end[1] <- mb_bin_end$mb_bin
      # cpPos$mb_bin_after[1] <- mb_bin_after$mb_bin
    }
    else {
      cpPos <- cpPos %>%
      ...
      for (i in 1:nrow(cpPos)) {
        ...
        # mb_bin_before <- blank_genome %>%
        #   dplyr::filter(start_chrom == cpPos$before_chrom[i] & start == cpPos$start_before[i])
        cpPos$mb_bin_start[i] <- mb_bin_start$mb_bin
        # cpPos$mb_bin_before[i] <- mb_bin_before$mb_bin
        mb_bin_end <- blank_genome %>%
          dplyr::filter(end_chrom == cpPos$end_chrom[i] & end == cpPos$end[i])
        # mb_bin_after <- blank_genome %>%
        #   dplyr::filter(end_chrom == cpPos$after_chrom[i] & end == cpPos$end_after[i])
        cpPos$mb_bin_end[i] <- mb_bin_end$mb_bin
        # cpPos$mb_bin_after[i] <- mb_bin_after$mb_bin
      }
    }
...
main <- function() {
    ...
    atac_test <- atacTest(type, tcga_code)
    if (tcga_code == "KIRC") {
      type <- "kidney-rcc-kirc"
    }
    if (tcga_code == "KIRP") {
      type <- "kidney-rcc-kirp"
    }
    write.csv(atac_test, file=paste0(type,"_atacpvals.txt"), row.names=FALSE)
}
