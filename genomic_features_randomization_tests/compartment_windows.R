...
# read in dataset of consensus AB compartments across the genome
raw <- read.delim('/home/timmonsc/testing/tcga_ab_compartments.txt')

# Automates testing
# Input folder containing signature profiles for a cancer type (expects that there is a matching file of ids
# that will direct to copy number profiles
...
compartmentsTest <- function(type, tcga_code) {
  pvals <- data.frame()

  compartments <- makeCompartments(raw, tcga_code)
    ...
    if (file.exists(paste0("/home/timmonsc/testing/",type,"/",type,i,".Rdata"))) {
      ...
      if (!is.null(get(temp)[[2]])||!is.null(get(temp)[[6]])||!is.null(get(temp)[[10]])||!is.null(get(temp)[[14]])||!is.null(get(temp)[[18]])) {
        ...
        if (nrow(changepoints_all) > 0) {
          ...
          null_dist <- getRandomChangepoints(get(temp), compartments)

          all <- testSampleChangepoints(changepoints_all, compartments) %>%
              dplyr::mutate(
                          "sampleID" = i,
                          "null_before" = 1,
                          "null_after" = 1)
...
# Find the A/B compartment distributions within random changepoint-containing segment
...
# Return p-values
getRandomChangepoints <- function(traj, compartments) {
  ...
  if (nrow(compartments) > 0) {
    ...
    before_segment_vals <- apply(before_segment, 1, function(x){compartments$domain[(which.min(abs(x[1]-compartments$mb_bin_start))):((which.min(abs((x[2]+1)-compartments$mb_bin_start)))-1)]})
    cp_segment_before_vals <- apply(cp_segment_before, 1, function(x){compartments$domain[(which.min(abs(x[1]-compartments$mb_bin_start))):((which.min(abs((x[2]+1)-compartments$mb_bin_start)))-1)]})
    cp_segment_after_vals <- apply(cp_segment_after, 1, function(x){compartments$domain[(which.min(abs(x[1]-compartments$mb_bin_start))):((which.min(abs((x[2]+1)-compartments$mb_bin_start)))-1)]})
    after_segment_vals <- apply(after_segment, 1, function(x){compartments$domain[(which.min(abs(x[1]-compartments$mb_bin_start))):((which.min(abs((x[2]+1)-compartments$mb_bin_start)))-1)]})

    before_segment_vals <- sapply(before_segment_vals, function(x){c(length(x[x=="open"]), length(x[x=="closed"]))})
    cp_segment_before_vals <- sapply(cp_segment_before_vals, function(x){c(length(x[x=="open"]), length(x[x=="closed"]))})
    cp_segment_after_vals <- sapply(cp_segment_after_vals, function(x){c(length(x[x=="open"]), length(x[x=="closed"]))})
    after_segment_vals <- sapply(after_segment_vals, function(x){c(length(x[x=="open"]), length(x[x=="closed"]))})
...
# make dataset of AB compartments across the genome given a raw file of compartment information
makeCompartments <- function(raw, code) {
  type_ab <- raw %>%
    dplyr::filter(tcga_code == tolower(code))
  type_ab <- type_ab %>%
    separate(chr, into=c("x1", "chr"), sep="r") %>%
    select(-x1) %>%
...
# Find the A/B compartment distributions within each changepoint-containing segment
# and regions surrounding changepoints
# Perform KS test between changepoint region and surrounding regions
# Return p values
testSampleChangepoints <- function(changepoints, compartments) {
  ...
  for (n in 1:nrow(changepoints)) {
    ...
    before_segment1 <- which.min(abs((midpoints_before[n]-(lengths_before[n]/2))-compartments$mb_bin_start))
    before_segment2 <- which.min(abs((midpoints_before[n]+(lengths_before[n]/2))-compartments$mb_bin_start))
    before_segment <- compartments$domain[before_segment1:before_segment2]
    before_segment <- c(length(before_segment[before_segment=='closed']), length(before_segment[before_segment=='open']))
    before_cp1 <- which.min(abs((midpoints_within[n]-(lengths_before[n]/2))-compartments$mb_bin_start))
    before_cp2 <- which.min(abs((midpoints_within[n]+(lengths_before[n]/2))-compartments$mb_bin_start))
    before_cp <- compartments$domain[before_cp1:before_cp2]
    before_cp <- c(length(before_cp[before_cp=='closed']), length(before_cp[before_cp=='open']))
    # find dist of atac scores after cp vs within cp
    after_segment1 <- which.min(abs((midpoints_after[n]-(lengths_after[n]/2))-compartments$mb_bin_start))
    after_segment2 <- which.min(abs((midpoints_after[n]+(lengths_after[n]/2))-compartments$mb_bin_start))
    after_segment <- compartments$domain[after_segment1:after_segment2]
    after_segment <- c(length(after_segment[after_segment=='closed']), length(after_segment[after_segment=='open']))
    after_cp1 <- which.min(abs((midpoints_within[n]-(lengths_after[n]/2))-compartments$mb_bin_start))
    after_cp2 <- which.min(abs((midpoints_within[n]+(lengths_after[n]/2))-compartments$mb_bin_start))
    after_cp <- compartments$domain[after_cp1:after_cp2]
    after_cp <- c(length(after_cp[after_cp=='closed']), length(after_cp[after_cp=='open']))
...
main <- function() {
  ...
  tcga_code = as.character(args[2])
  compartments_test <- compartmentsTest(type, tcga_code)
  write.csv(compartments_test, file=paste0("/home/timmonsc/testing/",type,"_compartmentpvals.txt"), row.names=FALSE)
}
