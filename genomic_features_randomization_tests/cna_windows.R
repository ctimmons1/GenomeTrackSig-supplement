...
# read in template of genomic coordinates binned by Mb
blank_genome <- readr::read_csv("blank_genome.csv")[,1:4]
...
# make dataset of copy number across the genome given a file of copy number calls
        ...
        if (!is.null(get(temp)[[2]])||!is.null(get(temp)[[6]])||!is.null(get(temp)[[10]])||!is.null(get(temp)[[14]])||!is.null(get(temp)[[18]])) {
          ...
          if (file.exists(paste0("/home/timmonsc/testing/consensus.20170119.somatic.cna.annotated/",fileID,".consensus.20170119.somatic.cna.annotated.txt"))) {
            ...
            cnas <- makeCnaDataset(fileID, blank_genome) %>%
              ...
              tidyr::drop_na()
            cna_bins <- sort(unique(unlist(mapply(function(x,y){rep(floor(x):floor(y))}, cnas$mb_bin_start, cnas$mb_bin_end))))
            cna_presence <- blank_genome %>%
              dplyr::mutate(cna = dplyr::if_else(mb_bin %in% cna_bins, 1, 0))

            null_dist <- getRandomChangepoints(fileID, get(temp), changepoints_all, cna_presence)

            full <- data.frame()
            truc <- data.frame()
            all <- testSampleChangepoints(changepoints_all, cna_presence) %>%
...
# Find the CNA distributions within each changepoint-containing segment
# and regions surrounding changepoints
# Perform KS test between changepoint region and surrounding regions
# Return p values
testSampleChangepoints <- function(changepoints, cna_presence) {
  if (nrow(changepoints) > 0) {
    ...
    for (n in 1:nrow(changepoints)) {
      ...
      before_segment1 <- which.min(abs((midpoints_before[n]-(lengths_before[n]/2))-cna_presence$mb_bin))
      before_segment2 <- which.min(abs((midpoints_before[n]+(lengths_before[n]/2))-cna_presence$mb_bin))
      before_segment <- cna_presence$cna[before_segment1:before_segment2]
      before_segment <- c(length(before_segment[before_segment==1]), length(before_segment[before_segment==0]))
      before_cp1 <- which.min(abs((midpoints_within[n]-(lengths_before[n]/2))-cna_presence$mb_bin))
      before_cp2 <- which.min(abs((midpoints_within[n]+(lengths_before[n]/2))-cna_presence$mb_bin))
      before_cp <- cna_presence$cna[before_cp1:before_cp2]
      before_cp <- c(length(before_cp[before_cp==1]), length(before_cp[before_cp==0]))
      # find dist of atac scores after cp vs within cp
      after_segment1 <- which.min(abs((midpoints_after[n]-(lengths_after[n]/2))-cna_presence$mb_bin))
      after_segment2 <- which.min(abs((midpoints_after[n]+(lengths_after[n]/2))-cna_presence$mb_bin))
      after_segment <- cna_presence$cna[after_segment1:after_segment2]
      after_segment <- c(length(after_segment[after_segment==1]), length(after_segment[after_segment==0]))
      after_cp1 <- which.min(abs((midpoints_within[n]-(lengths_after[n]/2))-cna_presence$mb_bin))
      after_cp2 <- which.min(abs((midpoints_within[n]+(lengths_after[n]/2))-cna_presence$mb_bin))
      after_cp <- cna_presence$cna[after_cp1:after_cp2]
...
# Find the CNA distributions within random changepoint-containing segment
...
# Return p-values
getRandomChangepoints <- function(fileID, traj, changepoints, cna_presence) {

  n = 10000

  if (nrow(cna_presence) > 0) {
    ...
    before_segment_vals <- sapply(before_segment, function(x){cna_presence$cna[x]})
    cp_segment_before_vals <- sapply(cp_segment_before, function(x){cna_presence$cna[x]})
    cp_segment_after_vals <- sapply(cp_segment_after, function(x){cna_presence$cna[x]})
    after_segment_vals <- sapply(after_segment, function(x){cna_presence$cna[x]})
...
main <- function() {
    ...
    cna_test <- cnaDist(type)
    write.csv(cna_test, file=paste0("/home/timmonsc/testing/",type,"_cnapvals.txt"), row.names=FALSE)
}
