...
# get dene density at 1 Mb windows throughout the genome
windows <- GenomicRanges::tileGenome(seqlengths = chrom_lengths,
                                     tilewidth = 1e3, cut.last.tile.in.chrom = TRUE)
...
dens <- dens[1:3095000]
density_data <- data.frame(density = dens, mb_bin = rep(1:3095, each=1000))
        ...
        changepoints_all <- summarizeChangepoints(get(temp), i) %>%
            ...
            if (nrow(changepoints_all) > 0) {
              ...
              changepoints_all <- rbind(changepoints_full, changepoints_truc) %>%
                dplyr::arrange(cpPos1)
        null_dist <- randomKS(get(temp), density_data)

          all <- sampleKS(changepoints_all, density_data) %>%
             dplyr::mutate(
                            "sampleID" = i,
                            "null_before" = 1,
                            "null_after" = 1)
...
# Find the gene density distributions within random changepoint-containing segment
...
# Return p-values
randomKS <- function(traj, density_data) {
  ...
  if (nrow(density_data) > 0) {
    ...
    before_segment_vals <- apply(before_segment, 1, function(x){density_data$density[(which.min(abs(x[1]-density_data$mb_bin))):((which.min(abs((x[2]+1)-density_data$mb_bin)))-1)]})
    cp_segment_before_vals <- apply(cp_segment_before, 1, function(x){density_data$density[(which.min(abs(x[1]-density_data$mb_bin))):((which.min(abs((x[2]+1)-density_data$mb_bin)))-1)]})
    cp_segment_after_vals <- apply(cp_segment_after, 1, function(x){density_data$density[(which.min(abs(x[1]-density_data$mb_bin))):((which.min(abs((x[2]+1)-density_data$mb_bin)))-1)]})
    after_segment_vals <- apply(after_segment, 1, function(x){density_data$density[(which.min(abs(x[1]-density_data$mb_bin))):((which.min(abs((x[2]+1)-density_data$mb_bin)))-1)]})
...
# Find the gene density distributions within each changepoint-containing segment
# and regions surrounding changepoints
# Perform KS test between changepoint region and surrounding regions
# Return p values
sampleKS <- function(changepoints, density_data) {
  changepoints <- changepoints %>%
    dplyr::filter(mb_bin_end <= 3095)
  # find mean gene dens halfway between segments before and after cp and halfway within cp segment
  ...
  for (n in 1:nrow(changepoints)) {
    ...
    lengths_before <- (starts - midpoints_before) / 2
    lengths_after <- (midpoints_after - ends) / 2
    # find mean and sd gene density in the middle of segment before changepoint
    before_segment1 <- which.min(abs((midpoints_before[n]-(lengths_before[n]/2))-density_data$mb_bin))
    before_segment2 <- which.min(abs((midpoints_before[n]+(lengths_before[n]/2))-density_data$mb_bin))
    before_segment <- density_data$density[before_segment1:before_segment2]
    before_cp1 <- which.min(abs((midpoints_within[n]-(lengths_before[n]/2))-density_data$mb_bin))
    before_cp2 <- which.min(abs((midpoints_within[n]+(lengths_before[n]/2))-density_data$mb_bin))
    before_cp <- density_data$density[before_cp1:before_cp2]
    # find mean and sd gene density in the middle of segment after changepoint
    after_segment1 <- which.min(abs((midpoints_after[n]-(lengths_after[n]/2))-density_data$mb_bin))
    after_segment2 <- which.min(abs((midpoints_after[n]+(lengths_after[n]/2))-density_data$mb_bin))
    after_segment <- density_data$density[after_segment1:after_segment2]
    after_cp1 <- which.min(abs((midpoints_within[n]-(lengths_after[n]/2))-density_data$mb_bin))
    after_cp2 <- which.min(abs((midpoints_within[n]+(lengths_after[n]/2))-density_data$mb_bin))
    after_cp <- density_data$density[after_cp1:after_cp2]
...
main <- function() {
    ...
    dens <- dens[1:3095000]
    density_data <- data.frame(density = dens, mb_bin = rep(1:3095, each=1000))
    density_test <- densityTest(type)
    write.csv(density_test, file=paste0("/home/timmonsc/testing/",type,"_genedenspvals.txt"), row.names=FALSE)
}
